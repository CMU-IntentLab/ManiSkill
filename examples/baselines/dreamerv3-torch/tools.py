...
def simulate(
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        for i in range(len(reward)):
            results.append((obs[i], reward[i], done[i], {'discount': np.array(1.0).astype(np.float32),'env_ids': info_vec['env_ids'][i]}))# 'safety_score': safety_score}))
        
        if isinstance(action, dict):
            action = [
                {k: action[k][i] for k in action}
                for i in range(envs.num_envs)
            ]
        else:
            action = np.array(action)
        assert len(action) == envs.num_envs

        for a, result in zip(action, results):
            o, r, d, info = result
            env_id = info['env_ids']
            ...
            add_to_cache(cache, env_id, transition)
        ...
        if done.any():
            ...
            for i in indices:
                ...
                video_wrist = cache[env_id]["wrist_cam"]
                video_front = cache[env_id]["front_cam"]

                #safety_score = cache[env_id]["safety_score"]
                #for i in range(len(safety_score)):
                #    if safety_score[i] < 0:
                #        video_wrist[i][:, :, 1] += 40
                #        video_front[i][:, :, 1] += 40

                # want to get the actions for this sequence
                actions = cache[env_id]["action"]
                #print('actions', np.shape(actions)) #(60, 7)
                video = np.concatenate((video_wrist, video_front), axis=1)
                #print('video', np.shape(video)) #(60, 480, 640, 6)
                # record logs given from environments
                for key in list(cache[env_id].keys()):
                    if "log_" in key:
                        logger.scalar(
                            key, float(np.array(cache[env_id][key]).sum())
                        )
                        # log items won't be used later
                        cache[env_id].pop(key)
                ...
                if not is_eval:
                    ...
                    print('eval_policy', np.shape(np.array(video)[None]))
