...
class WorldModel(nn.Module):
    ...
    def preprocess(self, obs):
        obs = {
            k: torch.as_tensor(v, device=self._config.device, dtype=torch.float32) #torch.tensor(v, device=self._config.device, dtype=torch.float32)
            for k, v in obs.items()
        }
        for k in obs.keys():
            if "cam" in k:
                obs[k] = torch.Tensor(obs[k]) / 255.0
        if "discount" in obs:
    ...
    def video_pred(self, data):
        ...
        recon = self.heads["decoder"](self.dynamics.get_feat(states))["wrist_cam"].mode()[
            :6
        ]
        reward_post = self.heads["reward"](self.dynamics.get_feat(states)).mode()[:6]
        init = {k: v[:, -1] for k, v in states.items()}
        prior = self.dynamics.imagine_with_action(data["action"][:6, 5:], init)
        openl = self.heads["decoder"](self.dynamics.get_feat(prior))["wrist_cam"].mode()
